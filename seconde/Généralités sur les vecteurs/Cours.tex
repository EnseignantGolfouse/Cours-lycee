...
\begin{vocabulaire}
	\begin{center}
		\begin{tikzpicture}
			...
			\node[above] at (A) {A};
			\node[above] at (B) {B};
			\draw[->] (A) -- node[midway,above left] {$\vec{u} = \vec{AB}$} (B);
		\end{tikzpicture}
	\end{center}

	Si une translation de vecteur $\vec{u}$ envoie le point A sur le point B, on peut appeller le vecteur de cette translation \squareFrame{$\vec{AB}$}. On dit alors que $\vec{AB}$ est un \textbf{représentant} de $\vec{u}$.
\end{vocabulaire}
