...
\newcommand{\Exercice}{
\begin{frame}
	ABCDEFGH est un octogone régulier de centre O. \begin{tikzpicture}
		\coordinate (O) at (0,0);
		\coordinate (A) at (1,0);
	...
	\begin{enumerate}
		...
		\item Indiquer à chaque fois si l'affirmation est vraie ou fausse :
		      \begin{multicols}{2}
			      \begin{itemize}
				      \item $\widevec{GH}$ et $\widevec{OB}$ sont égaux : \correction{FAUX}
				      \item $\widevec{FE}$ et $\widevec{BA}$ sont opposés : \correction{VRAI}
				      \item $\widevec{GF}$ et $\widevec{OE}$ sont opposés : \correction{FAUX}
				      \item $\widevec{AF}$ et $\widevec{DC}$ sont égaux : \correction{FAUX}
			      \end{itemize}
		      \end{multicols}
	\end{enumerate}
\end{frame}
