...
\title{Évaluation : Généralités sur les fonctions (sujet B)}
\author{}
\date{}
...
\begin{tcolorbox}
	...
	Ne pas oublier de mettre son nom et prénom sur sa copie, ainsi que le sujet (B).
...
\begin{exercice}
	...
	\begin{enumerate}
		\item Donner l'image par $f$ de $-2$ et $2$.
		\item Quelle est le sens de variation de $f$ sur $[1 ; 5]$ ?
		\item Calculer le taux de variation de $f$ entre $2$ et $5$.
		\item Donner l'image par $g$ de $-4$ et $2$.
...
\begin{exercice}
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.7]
			...
			\draw[<->] (Figure1) ++(3.2,0) -- node[right] {$x + 2$ cm} ++(0,4);
			...
			\draw[<->] (Figure2) ++(3.2,0) -- node[right] {$9$ cm} ++(0,2.1);
			\draw[<->] (Figure2) ++(3.2,2.1) -- node[right] {$7$ cm} ++(0,1.4);
