...
On considère une population de truites dans un lac. Chaque année, le taux d'évolution de la population de truites dû à la reproduction est de $15$\%.
...
\begin{enumerate}
	...
	\item On suppose que le nombre initial de truites est de $10\ 000$. Donner alors une définition de la suite $uₙ$ qui décrit le nombre de truites au bout de $n$ années :
	      ...
	      \end{align*}
	\item On suppose maintenant que chaque année, $10\%$ des truites sont pêchées par des humains (après l'augmentation dû à la reproduction).

	      Quelle est alors le taux d'évolution du nombre de truites d'année en année ? \medskip

	      \correctionOr{{\color{red}$1,15 × 0,9 = 1,035$. Le nombre de truites augmente donc de $3,5$\% par an.}}{\reponse} \medskip
